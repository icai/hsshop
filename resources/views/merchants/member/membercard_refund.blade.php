    ...
    <div class="content_header">
        <div class="pull-right search_module">
            ...
            <label class="search_items">
                <input class="search_input" type="text" id="search"  name="" value="{{$card_no or ''}}" placeholder="搜索"/>
            </label>
            <!-- 搜索 结束 -->
        </div>
    </div>
          ...
          <table class="ui-table ui-table-list">
            <thead>
                <tr class="widget-list-header">
                    <th class="cell-20 text-left">退卡时间</th>
                    <th class="cell-15">会员卡号</th>
                    <th class="cell-15">会员</th>
                    <th class="cell-15">会员卡
                        <span class="list-header-line">|</span>类型</th>
            ...
            @foreach($refund as $v)
                <tr class="widget-list-item">
                    <td class="text-left">{{$v['out_card_at']}}</td>
                    <td>{{$v['card_num']}}</td>
                    <td>
                        <a target="_blank" href="{{ URL('merchants/member/customer') }}"></a>{{$v['member_title']}}</td>
                    <td>
                        <p>{{$v['memberCard']['title']}}</p>
                        <p style="color: #9b9b9b">@if($v['memberCard']['card_status'] == 0) 无门槛会员卡 @else 按规则发放的会员卡 @endif</p>
                    </td>
                    <td></td>
                    <td>
                        @if($v['memberCard']['state'] == 1)
                            已启用
                        @elseif($v['memberCard']['state'] == -1)
                            已删除
                        @else
                            已禁用
                        @endif
