...
<div class="middle_header">
    <div class="third_nav">
        ...
        <ul class="crumb_nav clearfix">
            ...
            <li>
                <a href="{{URL('/merchants/marketing/scratchList')}}">刮刮卡</a>
            </li>
            <li>
...
<div class="content">	
	<ul class="screen_nav nav nav-tabs mgb15" role="tablist">
        <li role="presentation" @if(!request('status')) class="active" @endif>
            <a href="/merchants/marketing/scratchCount/{{Route::input('scratchId')}}">所有参与</a>
        </li> 
        <li role="presentation"  @if(request('status') == 2) class="active"@endif>
            <a href="/merchants/marketing/scratchCount/{{Route::input('scratchId')}}?status=2">中奖</a>
        </li>  
        <li role="presentation" @if(request('status') == 1) class="active"@endif>
            <a href="/merchants/marketing/scratchCount/{{Route::input('scratchId')}}?status=1">未中奖</a>
        </li>         
   	</ul>
   	<div class="searchDiv">
		<form class="flex_around" action="" method="get">
            ...
            <a href="/merchants/marketing/scratchCount/{{Route::input('scratchId')}}?is_export=1&status={{request('status')}}&name={{request('name')}}"><div class="btn btn-primary">批量导出</div></a>
		</form>
	</div>
    <!--无数据-->
