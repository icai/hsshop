...
<head>
	...
	.content-nav .price .title-price{
		font-size:26px
	}
	.content-nav .price .ex-price{
	...
	.content-nav .reset-content{
		padding:0 10px;
		text-align:center;
		position:relative;
		background:#FFAB32;
	}
	.content-nav .reset-content:after{
		content:'';
		position:absolute;
		top:0;
		left:-39px;
	...
	.content-nav .reset-content .reset-time-tip{
		font:12px/28px '微软雅黑';
		color:#C62121
	}
	.reset-time span{
...
<div id="app" v-cloak>
	<div class="page" v-if="show != ''">
		...
		<div class="content-nav flex-box">
			<p class="price">￥<span class="title-price">@{{list1.product.price}}</span><span class="ex-price">原价：￥16800</span></p>
			<div class="reset-content">
				<p class="reset-time-tip">距活动结束还剩</p>
				<p class="reset-time"><span>@{{oHour}}</span>：<span>@{{oMini}}</span>：<span>@{{oSec}}</span></p>
			</div>
		</div>
		...
		<div class="banner">
			<img src="{{ config('app.source_url') }}shop/images/meetingBaner.png">
		</div>
	...
	<div v-if="fwbz">
		...
		<div class="fwbzTc tc">
			...
			<div class="f_content">
				<div>
					<!-- <div class="f_tcSubTit" v-html="'<span></span>' + ftc.title"><span>·</span>全场包邮</div> -->
