...
app.controller('myCtrl',function($scope,$http,$timeout){
    ...
    * */
    $scope.$on('ngRepeatFinished', function (ngRepeatFinishedEvent) {
        ...
        for(var i = 0; i < dragBox.length; i++){
            dragBox[i].ondragstart = function (e) {
                console.log(e);
                e.dataTransfer.setData("imgInfo", "11111")
                $scope.dragId = $(this).attr('data-id')
        ...
        $(".dragBox").on('dragend',function (e) {
            $('.dragBox')[$scope.dragId].style.opacity = 1
        })
        $(".dragBox").on('drag',function (e) {
            // console.log('我在移动');
        })
        ...
        $(".dropBox").on('dragover', function(e){  //源对象在悬停在目标对象上时
        ...
        $(".dropBox").on('drop ',function (e) {
            ...
            if(dropId == $scope.dragId){
                return false
            }else if(dropId == 0){
                return false
            }else{
                var dropData = $scope.tabBarList[dropId]
                var dragData = $scope.tabBarList[$scope.dragId]
                $scope.safeApply(function () {
                    $scope.tabBarList[dropId] = dragData
                    $scope.tabBarList[$scope.dragId] = dropData
                })
            }
        })
    });
    $scope.dragId = null //保存被拖拽元素的索引
    ...
    $scope.selectChange = function($index){
        ...
        if($scope.tabBarList[$index]['grade'] == 'relation'){
            if($scope.eachData('门店') >= 1){
                tipshow('您已设置“门店”，请勿重复添加','warn');
                $scope.tabBarList[$index]['grade'] = '';
                return;
            }
            $scope.addNav(0);
	...
	*/
	...
	$scope.addNav = function(type,event){
		...
		switch (type) {
            case 0:
            	...
            	$scope.tabBarList[$scope.index]['urlTitle'] = "门店";
	...
	function saveDatabase(callback){
		$http.post("/merchants/marketing/SaveBar",{barList:$scope.tabBarList}).success(function (data) {
	 ...
	 * 页面编辑
	 */
	$http.post("/merchants/marketing/getBarDataList").success(function (data) {
        if(data.status == 1){
        	$scope.is_auth_submit = data.data.is_auth_submit;//自动更新赋值
        	$scope.isBinding = data.data.isBinding;//绑定小程序赋值
            $scope.xcx_status = data.data.xcx_status;
