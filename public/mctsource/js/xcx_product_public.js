...
//  获取享立减商品
    ...
    commonServer.newActivity = function($scope){
    	...
    	switch ($scope.activityIndex)
        {
            ...
            break;
            case 1://刮刮卡
        	    window.open('/merchants/marketing/addScratch');
            break;
        }
    }
    //end
    ...
    commonServer.switchNav = function($index,$scope){
        ...
        switch ($scope.activityIndex)
        {
            ...
            break;
            case 1://砸金蛋
                var url = "/merchants/marketing/getUsefulScratch?page=1";
            break;
        }
    ...
    commonServer.chooseActivity = function($index,position,$scope){
    	console.log($index,position,$scope)
...
//      $scope.link_type = 10;
    ...
    commonServer.chooseActivitySure = function($index,$scope,list){
        ...
        if($scope.shopLinkPosition == 1){
            ...
            console.log(linkUrl,$scope.link_type)
        ...
        //魔方添加营销活动入口
        	...
        	$scope.editors[$scope.index]['content'][$scope.cube.selectLayoutIndex]['type'] = $scope.link_type;//11为大转盘，12为刮刮卡
        ...
        //商品分组模板营销活动确定
        	...
        	console.log($scope.GroupLinkType)
    ...
    commonServer.sureSetLink = function($scope){
        var url = $('#wailink_input').val().substr(0,1) == "/" ? $('#wailink_input').val() : "/" + $('#wailink_input').val();
        if (url.substr(0,6) != '/pages'){
            tipshow('请添加小程序页面路径', 'warn');
            return;
        }
        if($scope.editors[$scope.index]['images']==undefined){
            ...
            $scope.editors[$scope.index]['images'][$scope.advImageIndex]['link_type'] = 16;
    ...
    commonServer.chooseShopLink = function($index,$scope,list){
        ...
        if($scope.shopLinkPosition == 1){
            ...
            //标题更新商品分组分支
            if(urlArr[2]=="group"){
                $scope.editors[$scope.index]['linkUrl'] = '/pages/activity/pages/productList/productList?typeId=' + list.id;
            }else{
                $scope.editors[$scope.index]['linkUrl'] = '/pages/main/pages/product/product_detail/product_detail?id='+ list.id;
            }
            $scope.editors[$scope.index]['chooseLink'] = true;
            $scope.editors[$scope.index]['dropDown'] = false;
        }else if($scope.shopLinkPosition == 3){
            ...
            $scope.editors[$scope.index]['textlink'][$scope.advsImagesIndex]['chooseLink'] = true;
            $scope.editors[$scope.index]['textlink'][$scope.advsImagesIndex]['dropDown'] = false;
            $scope.editors[$scope.index]['textlink'][$scope.advsImagesIndex]['link_type'] = $scope.link_type;
        }else if($scope.shopLinkPosition == 6){
            //冰冰模板背景链接选择
            $scope.editors[$scope.index]['linkName'] = list.name;
            $scope.editors[$scope.index]['linkUrl'] = list.url;
            $scope.editors[$scope.index]['linkType'] = 2;
            ...
            $scope.editors[$scope.index]['lists'][$scope.advsImagesIndex]['linkType'] = 2;
            $scope.editors[$scope.index]['lists'][$scope.advsImagesIndex]['id'] = list.id;
        }else if($scope.shopLinkPosition == 8){
            //魔方添加商品
                //添加商品分组
            if(urlArr[2]=="group"){
                $scope.editors[$scope.index]['content'][$scope.cube.selectLayoutIndex]['type'] = 17;
            }else{
    ...
    commonServer.choosePageLinkSure = function($index,list,$scope){
        if($scope.shopLinkPosition == 1){
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['linkName'] = list.name;
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['linkUrl'] = list.id;
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['link_id'] = list.id;
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['chooseLink'] = true;
//          $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['dropDown'] = false;
            if($scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['dropDown']){
                $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['dropDown'] = false;
            }else{
                $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['dropDown'] = true;
            }
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['link_type'] = $scope.link_type;
        }else if($scope.shopLinkPosition == 2){
            $scope.editors[$scope.index]['linkName'] = list.name;
            $scope.editors[$scope.index]['linkUrl'] = '/pages/micropage/index/index?id=' + list.id;
            $scope.editors[$scope.index]['chooseLink'] = true;
            $scope.editors[$scope.index]['dropDown'] = false;
        }else if($scope.shopLinkPosition == 3){
            //一级导航
            ...
            $scope.editors[$scope.index]['linkName'] = list.name;
            $scope.editors[$scope.index]['linkUrl'] = 'url';
            $scope.editors[$scope.index]['linkType'] = 1;
            $scope.editors[$scope.index]['id'] = list.id;
        }else if($scope.shopLinkPosition == 7){
            $scope.editors[$scope.index]['lists'][$scope.advsImagesIndex]['linkName'] = list.name;
            $scope.editors[$scope.index]['lists'][$scope.advsImagesIndex]['linkUrl'] = 'url';
            $scope.editors[$scope.index]['lists'][$scope.advsImagesIndex]['linkType'] = 1;
            $scope.editors[$scope.index]['lists'][$scope.advsImagesIndex]['id'] = list.id;
        }else if($scope.shopLinkPosition == 8){
            //魔方添加微页面
     ...
     // 图片广告选择大转盘链接确定
     commonServer.chooseWheelPageLinkSure = function($index,list,$scope){
        console.log($scope.shopLinkPosition,list,'都');
        if($scope.shopLinkPosition == 1){
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['linkName'] = list.name;
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['linkUrl'] = list.url;
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['link_id'] = list.id;
    ...
    commonServer.searchPage = function($scope){
     ...
     //搜索大转盘
     commonServer.searchWheelPage = function($scope){
        $scope.temp = [];
         // $scope.pageId = [];
        var wid = $('#wid').val();
        let e= {
        ...
        $scope.pageList = [];
        $.post('/merchants/marketing/wheelList?page=1&keyword=' + $scope.searchTitle, e, function(data) {
            data = data.data[0];
            if (data.data.length > 0) {
             ...
             var totalCount = data.total, showCount = 10,
                 ...
                 $('.page_pagenavi').extendPagination({
                     ...
                     limit: limit,
                     callback: function (page, limit, totalCount) {
                         $.post('/merchants/marketing/wheelList?page=' + page + '&keyword=' + $scope.searchTitle, e,function(response){
    ...
    // 图片广告选择刮刮乐活动确定
    commonServer.scratchCardSure = function($index,list,$scope){
    ...
    commonServer.chooseCubeCoupon = function($index,list,$scope){
        ...
        //商品分组模板优惠券确定
        	...
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['id'] = list.id;
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkName'] = list.name;
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkUrl'] = host + 'shop/activity/couponDetail/' + wid + '/' + list.id;
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['GroupLinkType'] = $scope.GroupLinkType;
    	}
     ...
     * @param {object} 页面数据 [$scope] 
     * @return: 
     * @Date: 2019-10-25 10:38:48
     * update: 戴江淮（npr5778@dingtalk.com）2019-10-25 新增会员卡选择图片
     * @update: 倪凯嘉（nikaijia@dingtalk.com）2020-03-12 新增直播组件添加封面图
     */ 
    commonServer.chooseAdvSureBtn = function($scope){
        ...
        if($scope.eventKind == 1){
            for(var i=0;i<$scope.tempUploadImage.length;i++){
                //增加图片尺寸限制 by倪凯嘉 2018.10.19
                let imgSize=$scope.tempUploadImage[i]['FileInfo'].img_size.split("x");
                ...
                if(imgWidth>=150){
                    ...
                    tipshow('图片尺寸小于150px，请重新上传','warm');
                }
            }
            ...
            if($scope.choosePosition == 1){
                ...
                if(imgWidth/imgHeight >= 0.8 && imgWidth/imgHeight <= 1.2){
           ...
           if($scope.tempUploadImage[0]['FileInfo']['path'].indexOf(imgUrl)>=0){
            ...
            console.log($scope.editors[$scope.index].content[$scope.cube.selectLayoutIndex].img)
        ...
        } else if ($scope.eventKind == 10) { // 直播组件添加封面图
          console.log( $scope.tempUploadImage, 'live-----------')
          let imgSize = $scope.tempUploadImage[0]['FileInfo'].img_size.split("x");
          let imgWidth = imgSize[0];//图片实际宽度
          if (imgWidth >= 450){
            if($scope.tempUploadImage[0]['FileInfo']['path'].indexOf(imgUrl)>=0){
              $scope.editors[$scope.index].img = $scope.tempUploadImage[0]['FileInfo']['path'];
            }else{
              $scope.editors[$scope.index].img = imgUrl + $scope.tempUploadImage[0]['FileInfo']['path'];
            }
          } else {
            tipshow('图片宽度不能小于450px，请重新上传','warm')
          }
        }
        $scope.hideModel();
    }
    ...
    commonServer.chooseLinkUrl = function($event,$index,position,url,linktype,$scope){
        ...
        if(position==1){
            ...
            $scope.editors[$scope.index]['images'][$index]['linkUrl'] = 1;
            $scope.editors[$scope.index]['images'][$index]['linkName'] = $event.target.text;
            $scope.editors[$scope.index]['images'][$index]['chooseLink'] = true;
            $scope.editors[$scope.index]['images'][$index]['link_type'] = linktype;
            ...
            $scope.editors[$scope.index]['linkType'] = linktype;
        }else if(position == 7){
            ...
            $scope.editors[$scope.index]['lists'][$index]['linkType'] = linktype;
        }
 ...
 //链接删除
            ...
            $scope.editors.push(
                {
                    ...
                    'editing':'editing',
                    'top_nav':[],//顶部菜单
                    'show_type':1, //1样式1，2样式2,3样式3
                    'isall': 1,//显示个数，2全部
                    'left_nav':[],//左侧菜单
                    'group_type':2,//1为左侧菜单，2为顶部菜单
                    'show_num':'',//显示个数为1的时候有值
                    'is_add_content':false,
                    'listStyle': 0
                }
            );
    ...
    commonServer.chooseGroupSure = function($scope){
        ...
        console.log($scope.temp);
        if($scope.temp.length>0){
            // if($scope.editors[$scope.index]['top_nav'].length + $scope.temp.length>8){
            //     tipshow('分组数量不能大于8个','warn');
            //     hideModel($('#goodslist_model'));
            //     return;
            // }
            if($scope.editors[$scope.index]['top_nav'].length + $scope.temp.length > 8 && $scope.editors[$scope.index]['group_type'] == 2){
                tipshow('分组数量不能大于8个','warn');
                hideModel($('#goodslist_model'));
                return;
            }
            //add by 邓钊 2018-8-13 注释商品分组左侧导航个数限制
            // if($scope.editors[$scope.index]['left_nav'].length + $scope.temp.length > 8 && $scope.editors[$scope.index]['group_type'] == 1){
            //     tipshow('分组数量不能大于8个','warn');
            //     hideModel($('#goodslist_model'));
            //     return;
            // }
            //end
            // if($scope.editors[$scope.index]['left_nav'].length > 8){
            //     tipshow('分组数量不能大于8个','warn');
            //     hideModel($('#goodslist_model'));
            //     return;
            // }
    ...
    */
    ...
    function killListAdd($scope,wid,title){
        $.get('/merchants/linkTo/get?type=13&platform=2&wid='+ wid +'&page=1&title='+title, function(data) {
            $scope.$apply(function(){
    ...
    commonServer.killModelShow = function($scope,position){
        var wid = $('#wid').val();
    ...
    commonServer.chooseKill = function(index,list,$scope){
        console.log($scope.skillLinkPosition)
         ...
         */
    		...
    		$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkUrl'] = host + "shop/seckill/detail";
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkName'] = list.activeName;
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['GroupLinkType'] = $scope.GroupLinkType;
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['id'] = list.id;
    	}
        hideModel($("#kill_model"));
    ...
    commonServer.chooseSpellModelSure = function($index,list,$scope){
        ...
        //商品分组模板页
        	...
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['GroupLinkType'] = $scope.GroupLinkType;
        	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['id'] = list.id;
        	console.log($scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex])
        }
        hideModel($('#page_model_pintuan'));
    }
    ...
    commonServer.chooseSpell_sure = function($index,list,$scope){
        if($scope.spellLinkPosition == 2){//魔方
            ...
            $scope.editors[$scope.index]['content'][$scope.cube.selectLayoutIndex]['id'] = list.id;
            $scope.editors[$scope.index]['content'][$scope.cube.selectLayoutIndex]['linkTitle'] = list.title;
        }else if($scope.spellLinkPosition == 3){//图片广告选择拼团
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['linkUrl'] = list.url;
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['linkName'] = list.title;
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['link_id'] = list.id;
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['chooseLink'] = true;
            ...
            $scope.editors[$scope.index]['images'][$scope.advsImagesIndex]['link_type'] = $scope.link_type;
            console.log(list);
            console.log($scope.editors[$scope.index]['images'])
        }
            ...
            $scope.editors.push(
                {
                    ...
                    "telType":0,    //模板类型 默认为0  0~7  1行两个~自定义
                    "margin": 0,    // 图片间隙
    ...
    commonServer.addOneTextLink = function($scope){
        $scope.editors[$scope.index]['textlink'].push({'titleName':'','dropDown':false,'linkName':'','linkUrl':'','link_type':''})
    }
    ...
    function researchAdd($scope,wid,title,type){
        var infoType = '';
        // 不传activity_type全部留言板展示
        if(typeof type !== 'undefined'){
            infoType = '&activity_type='+type
        }
        // 根据type不同 跳转至不同页面 0:在线报名,1:预约,2:投票 不传你就在营销页呆着吧
        if(typeof type == 'number'){
            $('.new_window').attr({href:'/merchants/marketing/researches/'+type})
        }else{
            $('.new_window').attr({href:'/merchants/marketing'})
        }
        $.get('/merchants/linkTo/get?type=19&platform=2&wid='+ wid +'&page=1&title='+title+infoType, function(data) {
            $scope.$apply(function(){
            ...
            $('.page_pagenavi').extendPagination({
                ...
                limit: limit,
                callback: function (page, limit, totalCount) {
                    $.get('/merchants/linkTo/get?type=19&wid='+ wid +'&page='+page+'&title='+title+infoType,function(response){
                        if(response.status ==1){
                            $scope.pageList = [];
                            console.log(response);
                            angular.forEach(response.data[0].data,function(val,key){
    ...
    commonServer.sureKill =function($scope){ 
         ...
         */
            ...
            $scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['GroupLinkType'] = $scope.GroupLinkType;
            $scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['id'] = $scope.temp[$scope.temp.length-1].id;
        }        
        hideModel($("#kill_model"));
    }
    /* add by 韩瑜
    ...
    commonServer.GroupChooseActivity = function($scope,p_index,$index,position){
        ...
        {
            case 0://营运大转盘
                var url = host + "merchants/marketing/wheelList?pagesize=6";
                $scope.GroupLinkType = 7;
            break;
            case 1://刮刮卡
                var url = host + "merchants/marketing/getUsefulScratch?page=1";
                $scope.GroupLinkType = 8;
            break;
        }
        console.log(url);
        $.post(url,{_token:_token},function(data){
            ...
            angular.forEach(data.data[0].data,function(val,key){
                ...
                $scope.$apply(function(){
                    $scope.productModal.list.push({
                        ...
                        "timeDay":val.created_at,
                        "timestamp":"" ,
                        "url":host + shopUrl+val.wid+'/'+val.id 
                    })
                })
            })
            var totalCount = data.data[0].total, showCount = 10,
            ...
            angular.forEach(data.data.data,function(val,key){
                $scope.$apply(function(){
                    $scope.pageList.push({
                        ...
                        "activityId":val.id,
                        "name":val.title,
                        "url": host + 'shop/product/detail/' + wid + '/' + val.product_id + '?activityId=' + val.id,
                        "created_at":val.created_at
                    })
                })
            })
    ...
    commonServer.GroupShareEventSure = function($scope,$index,list){
    	...
    	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkName'] = list.name;
    	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['GroupLinkType'] = $scope.GroupLinkType;
    	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['id'] = list.activityId;
    ...
    commonServer.GroupLinkUrl = function($event,p_index,$index,url,type,$scope){
    	...
    	if(type == 8){//店铺主页
	    	...
	    	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['GroupLinkType'] = 9;
    	}else if(type == 9){//会员主页
			$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkUrl'] = 'url';
	    	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkName'] = $event.target.text;
	    	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['GroupLinkType'] = 10;
    	}else if(type == 10){//购物车
			$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkUrl'] = 'url';
	    	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['linkName'] = $event.target.text;
	    	$scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$scope.advsImagesIndex]['GroupLinkType'] = 11;
    	}
    }
    commonServer.deleteGroupLink = function($scope,p_index,$index){
	    ...
	    $scope.editors[$scope.index]['classifyList'][$scope.Pindex]['subClassifyList'][$index]['GroupLinkType'] = '';
    }
     /**
     ...
     */    
    commonServer.addLive = function($scope,position){
      console.log('----------')
      $scope.removeClassEditing();
      if(position == 1){
          $scope.editors.push(
      ...
      $scope.initCartRight();
      console.log($scope.editors, 'live');
    }
    /**
     * @author: 倪凯嘉（nikaijia@dingtalk.com）
     ...
     */    
    commonServer.showLiveDialog = function($scope,type) {
      console.log($scope)
      $scope.liveList = []
      $scope.temp = []
      var wid = $('#wid').val();
      ...
      $.get('/merchants/xcx/liveRoom?type=1&wid='+ wid +'&page=1&flush=' + flush, function(data) {
        console.log(data,'直播间列表')
        if (data.errCode == 0) {
          angular.forEach(data.data.data,function(val,key){
              $scope.$apply(function(){
                  $scope.liveList.push({
                    ...
                    'anchor_img': val.anchor_img,
                    'live_status': val.live_status,
                    'id': val.id
                  })
              })
          })
          console.log($scope.liveList,'dddddddd');
          var totalCount = data.data.total, showCount = 10,
              limit = data.data.per_page;
              // alert(totalCount)
              $('.good_pagenavi').extendPagination({
          ...
          showModel($('#live'),$('#live-dialog'));
        } else if (data.errCode == 41026) { // 未授权
          console.log('未授权')
          showModel($('#unauthorized'),$('#unauthorized-dialog'));
        }
      },'json')
    }
    /**
     ...
     */    
    commonServer.chooseLive = function($scope, list) {
      console.log(list,'live-list')
      var obj = angular.copy(list)
      $scope.temp = obj
    }
    /**
     ...
     */    
    commonServer.selectLiveRoom = function($scope) {
      hideModel($('#live'))
      $scope.editors[$scope.index]['room'] = $scope.temp
      console.log($scope.editors[$scope.index])
    }
    /**
     * @author: 倪凯嘉（nikaijia@dingtalk.com）
     ...
     */    
    commonServer.cancelRoom = function($scope) {
      $scope.editors[$scope.index]['room'] = {}
    }
    /**
     * @author: 倪凯嘉（nikaijia@dingtalk.com）
     * @description: 删除已有封面图
     ...
     */    
    commonServer.cancelCoverImg = function($scope) {
      $scope.editors[$scope.index]['img'] = ''
    }
    /**
     * @author: 倪凯嘉（nikaijia@dingtalk.com）
     * @description: 添加封面图
     ...
     */    
    commonServer.addLiveCoverImg = function($scope) {
      $scope.uploadShow = false;
      $scope.grounps = [];
      $scope.eventKind = 10;
      $http.get('/merchants/myfile/getClassify').success(function(data){
