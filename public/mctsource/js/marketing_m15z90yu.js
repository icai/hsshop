$(function(){	
	...
	//优惠券
	var cou_lgth = couponList;
	for(var i = 0;i<cou_lgth.length;i++){
		var option = '<option value="'+cou_lgth[i].id+'">'+cou_lgth[i].title+'</option>';
...
//	编辑有数据显示
	...
	if($.isEmptyObject(data)==false){
		...
		$('#no_wrap').val(data.unwin_descr);//未中奖说明
		...
		$('.type1').find("input[type='radio'][value='"+data.prize[0].type+"']").attr("checked",true)//奖项类型
		var type1 = data.prize[0].type;
		$('.type2').find("input[type='radio'][value='"+data.prize[1].type+"']").attr("checked",true)//奖项类型
		var type2 = data.prize[1].type;
		$('.type3').find("input[type='radio'][value='"+data.prize[2].type+"']").attr("checked",true)//奖项类型
		var type3 = data.prize[2].type;
        $('.type4').find("input[type='radio'][value='"+data.prize[3].type+"']").attr("checked",true)//奖项类型
        var type4 = data.prize[3].type;
        ...
        $('.prize-content-set4 .prize-group').hide();
        $('.prize-content-set4 .points-st'+type4).show();
        $('.prize-content-set4 .points-st'+type4).find('.ctx-con').val(data.prize[3].content);
        $('.prize-content-set4 .points-st'+type4).find('.ctx-num').val(data.prize[3].num);
		//奖项内容
		//一等奖
	...
	//		三等奖
        ...
        //		普通奖
        var lgth4 = $('.type4').find("input[type='radio']:checked").index();
        $('.prize-content-set4 .prize-group').eq(lgth4).find('.tri-con').val(data.prize[3].content);
        $('.prize-content-set4 .prize-group').eq(lgth4).find('.tri-num').val(data.prize[3].num);
        $('.prize-content-set4 .prize-group').eq(3).find('.sort').removeClass('hide');
        $('.prize-content-set4 .prize-group').eq(3).find('.sort img').attr('src',_host + data.prize[3].img);
        $(".shuoming4").val(data.prize[3].method);
        $(".ctx-img").val(data.prize[3].img);
        $('.z-imgd').css('background-image','url("'+data.prize[3].img+'")');
	}
...
//	设置日期
    ...
    //奖品详情
    var t_indexc =$('.type4').find("input[type='radio']:checked").val();
    $('.type4').find("input[type='radio']").change(function(){
        t_indexc = $(this).val();
    })
    $('.ctx-con').blur(function(){
        if($('.prize-content-set3 .points-st'+t_indexc).find('.ctx-con').val()==""){
            $('.rem10').remove();
            $('.prize-content-set3 .points-st'+t_indexc).find('.ctx-con').css('border-color','red').after('<p class="nul-red rem10">奖品详情未填写</p>');
        }else{
            $('.rem10').remove();
            $('.ctx-con').css('border-color','#ccc');
        };
    });
    ...
    //奖品数量
    $('.ctx-num').blur(function(){
        if($('.prize-content-set3 .points-st'+t_indexc).find('.ctx-num').val()==""){
            $('.rem13').remove();
            $('.prize-content-set3 .points-st'+t_indexc).find('.ctx-num').css('border-color','red').next('span').after('<p class="nul-red rem13">奖品数量未填写</p>');
        }else{
            $('.rem13').remove();
            $('.ctx-num').css('border-color','#ccc');
        };
    });
...
//  	验证
		...
		if(n==2){
			...
			if($('.prize-content-set1 .points-st'+t_index).find('.fir-con').val()==""){
				tipshow("请填写一等奖，二等奖，三等奖，普通奖的赠送积分","warn");
				$('.rem8').remove();
				$('.prize-content-set1 .points-st'+t_index).find('.fir-con').css('border-color','red').after('<p class="nul-red rem8">奖品详情未填写</p>');
				return false;
			}
            ...
            if($('.prize-content-set4 .points-st'+t_indexc).find('.ctx-con').val()==""){
                tipshow("请填写一等奖，二等奖，三等奖，普通奖的赠送积分","warn");
                $('.rem10').remove();
                $('.prize-content-set4 .points-st'+t_indexc).find('.ctx-con').css('border-color','red').after('<p class="nul-red rem10">奖品详情未填写</p>');
                return false;
            };
			if($('.prize-content-set1 .points-st'+t_index).find('.fir-num').val()==""){
				$('.rem12').remove();
            ...
            if($('.prize-content-set4 .points-st'+t_indexc).find('.ctx-num').val()==""){
                $('.rem14').remove();
                $('.prize-content-set4 .points-st'+t_indexc).find('.ctx-num').css('border-color','red').next('span').after('<p class="nul-red rem14">奖品数量未填写</p>');
                return false;
            };
			if(!$('#no_wrap').val()){
                tipshow("请填写未中奖说明","warn");
                return false;
			}
...
//		三等奖
        ...
        prize[3].id=id_4;
        prize[3].grade=4;
        var t_indexd = $('.type4').find("input[type='radio']:checked").val();
        prize[3].type= t_indexd;
        prize[3].content = $('.prize-content-set4 .points-st'+t_indexd).find('.ctx-con').val();
        prize[3].num = $('.prize-content-set4 .points-st'+t_indexd).find('.ctx-num').val();
        prize[3].method = $(".shuoming4").val();
        prize[3].img = $(".ctx-img").val().replace(_host,'');
...
//		发送数据
		...
		$.ajax({
			url:"/merchants/marketing/saveScratch",
			type:"POST",
			data:data,		
			dataType:'json',
			headers: {
			...
			success:function(res){
				...
				if(res.status==1){
					...
					$('.miaoshu').html(descr);				
					//访问显示二维码
					var url = "pages/scratchCard/scratchCard?scratchId=" + res.data;
					$('.cop-int').val(url);
					$.ajax({
						type:"post",
                        ...
                        success:function(data){
							if(data.errCode == 0){
                                var img64 = 'data:image/png;base64,'+data.data
                                $("#img_xcxm").attr("src",img64);
							}
						},
						error:function(){
							alert("数据访问错误")
