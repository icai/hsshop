...
//  
//});
//$(document).on('click','.close_circle',function(){
//	$(this).parent().remove();
//});
////删除回复
...
//}
////添加文本
//$(document).on('click','.rule_add_reply .reply_cap_btn .btn-primary',function(){
//	var _that = $(this);
//	_that.prop('disabled',true);
//  edit_reply = _that.parents('.reply');
//	if ($(this).parents('.rule_add_reply').siblings('.reply_list').find('.reply').length == 0) {
//		$(this).parents('.rule_add_reply').siblings('.reply_list').show();
//		$(this).parents('.rule_add_reply').siblings('.right_info').hide();
//	}
//	//	判断要添加的是文本图片还是图文
//  var rule_id = _that.parents('.rule_right').data('rule_id');//自动回复的对象id
//	if($('.ctts textarea').css('display') != 'none'){
...
//      }
//	}
//	$(this).parents('.reply_cap').remove();
//});
////字数控制
//function fontCount(){
//	$('.reply_cap_btn').children('span').text("还能输入"+(300-$('.reply_cap_text').val().length)+"个字")
//}
...
//	
//});
//$(document).on('click','.qqFace img',function(){//点击表情聚焦文本框
...
*参数从左到右 自动回复对象的标识 表情图片等类型 发送的内容 成功后需要添加的内容 当前函数上下文
*             商品或图文唯一标识  其他中的类型
* type: 1 文本 2 图片 3语音 4 音乐 5 图文 6 其他  7客服
* content_type: 1商品 2微页面 3店铺主页 4 会员主页 
* id   编辑时传 id 
 */
function replyAjax(rule_id,type,configs,that,id,content_id,content_type){
