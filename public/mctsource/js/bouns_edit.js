...
app.controller('myCtrl',['$scope','$http',function($scope, $http) {
    ...
    $scope.tip_coupon = 0 //优惠券状态 0 正常 1过期 2失效 3库存为空
    $scope.tip_style = '' //优惠券状态样式
    //编辑渲染数据
    if(editData){
        console.log(editData);
        $scope.bounsTitle = editData.title
        $scope.start_at = editData.start_at
        $scope.end_at = editData.end_at
        $scope.microPage = { //微页面数据
            title:editData.micro_page_title,
            id:editData.micro_page_id,
            created_at:'',
            is_home:''
        }
        $scope.shop_microPage = { //微商城微页面数据
            title:editData.shop_micro_page_title,
            id:editData.shop_micro_page_id,
            created_at:'',
            is_home:'',
            url:''
        }
        if(editData.image){
            $scope.imageUrl =  imgUrl + editData.image
        }
        if(editData.show_interval != 0){
            $scope.grantVal = editData.show_interval
            $scope.grantType = 1
        }
        var newDate = Date.parse(new Date())
        console.log(newDate);
        var list = editData.coupons
        for(var i = 0; i < list.length; i++){
            var start_at = list[i].start_at.split(' ')
            var end_at = list[i].end_at.split(' ')
            if(Date.parse(new Date(list[i].end_at)) < newDate){
                $scope.tip_coupon = 1
                $scope.tip_style = 'past_due'
            }else if(list[i].invalid_at){
                $scope.tip_coupon = 2
                $scope.tip_style = 'lose_efficacy'
            }else if(list[i].left == 0){
                $scope.tip_coupon = 3
                $scope.tip_style = 'inventory'
            }
            if(list[i].range_type == 0){
                var range_type_title = '全品类'
            }else if(list[i].range_type == 1){
                var range_type_title = '部分劵'
            }
            if(list[i].is_limited == 0){
                var limit_desc = '无限制'
            }else if(list[i].is_limited == 1){
                var limit_desc = '满'+ list[i].limit_amount +'元可用'
            }
            $scope.coupon_li.push({
                ...
                range_type:list[i].range_type,
                id:list[i].id,
                tip_coupon:$scope.tip_coupon,
                tip_style:$scope.tip_style
            })
        }
    }
    /*
...
* */
    var start = {
        ...
        choose: function(datas){
            $('#startTime').attr("value",datas,id);
            $scope.startTime = datas
            var end_at = $('#endTime').val()
            timesFlag(end_at,datas,id)
    ...
    var end ={
        ...
        istoday: false,
        choose: function(datas){
            $('#endTime').attr("value",datas,id);
            var start_at = $('#startTime').val()
            timesFlag(datas,start_at,id)
        }
    };
    laydate(start);
    laydate(end);

    function timesFlag(end_at,start_at,id) {
        if (!end_at || !start_at) {
            return false
        } else {
            $.ajax({
    ...
    * */
    $scope.submit = function(){
        ...
        $.ajax({
            type:'post',
            url:'/merchants/marketing/bonus/edit/' + id,
            data:data,
