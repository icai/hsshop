$(function(){
	...
	if(GetQueryString('id')){//存在id可编辑
        ...
        if(shareEvent.rule_img){
            ...
            $('.image-wrap-rule .image-display').css('background-image','url("'+imgUrl+'hsshop/image/static/xianglijian_rule.jpg")');
        }
	...
	$('.js-btn-save').click(function(){
		...
		$.ajax({
			type:"POST",
			url:"/merchants/share/event/save",
			data:data,
			headers: {
            ...
            success:function(res){
            	if(res.status == 1){
            		tipshow(res.info);
            		setTimeout(function(){
            			window.location.href='/merchants/share/event/index'
            		},1000)
            	}else{
