...
var app = new Vue({
    ...
    created: function() {
        ...
        this.$http.get(url).then(function(res) { 
            ...
            var timeStamp = dataTop.end_time.split(/[- : \/]/);
            var resetTimeEnd = +new Date(timeStamp[0], timeStamp[1]-1, timeStamp[2], timeStamp[3], timeStamp[4], timeStamp[5]);
            var ntime = Date.now()
            this.getrtime(ntime,resetTimeEnd,0);
            // 客服url
        ...
        this.$http.get(url2).then(function(res) {
            ...
            for(var i = 0; i < this.list2Data.length; i++) {
                ...
                runTime = parseInt((d2 - d1) / 1000) % (86400 * 365) % (86400 * 30);
...
//                  }
        ...
        getrtime: function(ntime, etime, time) {
            ...
            var t = etime - ntime;
                ...
                that.oSec=s;
                that.oMini=m;
                that.oHour=parseInt(h) + parseInt(d)*24;
                setTimeout(function() {
...
function input(val){
    var div=$("<div><img src="+val.headimgurl+">"+val.nickname+","+val.sec+"秒前拼单了这个商品</div>");
    $(".d_show").append(div);
}
