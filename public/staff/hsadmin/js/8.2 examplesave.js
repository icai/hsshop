$(function(){
	...
	$("#sub").click(function () {
        $.ajax({
            url:'/staff/example/save',// 跳转到 action
	...
	$("body").on('click','.delImga',function () {
		var obj = $(this);        
	...
	$("body").on('click','.del-imga',function(){
		var imgarr = $("#attachmenta").val().split(";");
		var index = $(this).parents(".del-div").index();	
		$(".del-div").eq(index).remove();
		imgarr.splice(index,1);
		imgarr = imgarr.join(";")
		$("#attachmenta").val(imgarr);
	});
