...
use App\Model\MarketingActivityRuleWheel;
use App\Services\Marketing\ActivityRuleAbstract;

class WheelRulesService extends ActivityRuleAbstract{
    /**
     * 为活动添加规则
     * @param array $rules
     */
    public function addRules($rules = []){
        if($rules){
            ...
            foreach($rules as $rule){
                $sort++;
                MarketingActivityRuleWheel::insert([
    ...
    public function getRules(){
        return MarketingActivityRuleWheel::where('activity_id', '=', $this->getActivityId())
    ...
    public function deduct($rule_id, $num){
        //递减规则中剩余奖品数
        MarketingActivityRuleWheel::where(['id'=>$rule_id])->decrement('remaining', $num);
    }
}