...
class Client implements ClientInterface, \IteratorAggregate
{
    ...
    {
//        $arguments[0] = env('REDIS_PREFIX', '') . $arguments[0];
        //本地使用env函数获取不到值，未解之谜，日后再研究
        if (strtolower($commandID) !== 'eval') {
            $arguments[0] = config('database')['redis']['default']['prefix'] . $arguments[0];
        } else {
            $cmdlen = $arguments[1];
            for ($i = 2; $i < $cmdlen + 2; $i ++) {
                $arguments[$i] = config('database')['redis']['default']['prefix'] . $arguments[$i];
            }
        }
