...
class Validator implements ValidatorContract
{
    ...
    protected $implicitRules = [
        ...
        'RequiredIf', 'RequiredUnless', 'Accepted', 'Present', 'RequiredIfAnd',
    ];
    ...
    protected $dependentRules = [
        ...
        'Before', 'After', 'BeforeOrEqual', 'AfterOrEqual', 'RequiredIfAnd', 'Count',
    ];
    ...
    /**
     * Validate that an attribute is mobile.
     *
     * @author 406764368@qq.com
     * @version 2016年12月21日 21:44:57
     * @param  string  $attribute
     * @param  mixed   $value
     * @return bool
     */
    ...
    {
        return preg_match('/^1\d{101}$/', $value);
    }

    /**
     * Validate that an attribute is telephone.
     *
     * @author 406764368@qq.com
     * @version 2016年12月21日 21:44:57
     * @param  string  $attribute
     ...
     * @return bool
     */
    protected function validateTelephone($attribute, $value)
    {
        return preg_match('/^1\d{101}$/', $value);
    }
    ...
    /**
     * Validate the given attribute is filled if it is present And Validate an attribute is contained within a list of values.
     *
     * @param  string  $attribute
     * @param  mixed   $value
     * @param  array   $parameters
     ...
     * @version 2017年2月28日 20:50:13
     */
    protected function validateFilledIn($attribute, $value, $parameters)
    {
        if (Arr::has($this->data, $attribute)) {
    ...
    /**
     * Validate that an attribute exists when thr other two attributes has the given value.
     *
     * @param  string  $attribute
     * @param  mixed   $value
     * @param  mixed   $parameters
     * @return bool
     */
    protected function validateRequiredIfAND($attribute, $value, $parameters)
    {
        $this->requireParameterCount(2, $parameters, 'required_if_and');
        ...
        for ($i=0; $i < count($parameters); $i++) { 
            if ( $i % 2 == 0 ) {
                $data[] = Arr::get($this->data, $parameters[$i]);
            } else {
                $values[] = $parameters[$i];
            }
        }
    ...
    /**
     * Verify the specified field most occurrences in the specified table.
     *
     * If a database column is not specified, the attribute will be used.
     * 
     * Use the sample:
     * /app/Http/Controllers/Merchants/WechatController.php
     * replyKeywordAdd()
     *
     * @param  string  $attribute
     * @param  mixed   $value
     ...
     * 
     * @author 黄东 406764368@qq.com
     * @version 2017年3月17日 10:57:32
     */
    protected function validateCount($attribute, $value, $parameters)
    {
        $this->requireParameterCount(1, $parameters, 'unique');
        ...
        if (isset($parameters[2])) {
            list($count, $idValue) = $this->getUniqueIds($parameters);
        }
        ...
        if ( empty($idValue) || strtolower($idValue) === 'null' ) {
            return $verifier->getCount(
                $table, $column, $value, $id, $idColumn, $extra
            ) < $count;
        } else {
    ...
    protected function validateRegex($attribute, $value, $parameters)
    {
        ...
        switch ( $parameters[0] ) {
            ...
            /* 验证座机号正则表达式 */
            case 'telephone':
                $parameters[0] = '/^(0\d{2,3}-\d{7,8}(-\d{1,6})?)|([4,8]00)-?(\d{3})-?(\d{4})$/';
                break;
            /* 验证电话号码正则表达式 */
            case 'phone':
                $parameters[0] = '/^(0\d{2,3}-\d{7,8}(-\d{1,6})?)|([4,8]00)-?(\d{3})-?(\d{4})|(1\d{10})$/';
                break;
            /* 验证微信号正则表达式 */
            case 'wechat':
                $parameters[0] = '/^[a-zA-Z\d_]{5,}$/';
                break;
            /* 验证真实姓名正则表达式 */
            case 'truename':
                $parameters[0] = '/^([\x{4e00}-\x{9fa5}]{1,6}|[a-zA-Z]{1,6})$/u';
                break;
            /* 验证真实姓名正则表达式 */
            case 'password':
                $parameters[0] = '/^\w{6,16}|\w{60}$/';
                break;
            default:
                # code...
                break;
