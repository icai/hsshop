...
class Builder
{
     ...
     * @return $this
     */
    public function alias($table_as)
    {
        $this->alias = $table_as;
     ...
     * @version 2016年12月20日 11:59:43
     * @param  array  $column
     * @param  string  $boolean
     * @param  string  $method
     * @return $this
     * @update 陈文豪 2018年09月11日 处理返回where条件顺序错误的问题
     */
    public function wheres($column, $boolean = 'and', $method = 'where')
    {
        return $this->whereNested(function ($query) use ($column, $method) {
            foreach ($column as $key => $value) {
                if (is_string($key)) {
                    if ( $key == '_string' ) {
                        ...
                        switch ( $value[0] ) {
                            ...
                            case 'in':
                                ...
                                break;
                            case 'not in':
                                $this->whereNotIn($key, $value[1]);
                                break;
                            case 'column':
                                $temp = explode(' ', $value[1]);
                                if ( count($temp) == 2 ) {
                                    $this->whereColumn($key, $temp[0], $temp[1]);
                                } else {
                                    $this->whereColumn($key, $temp[0]);
                                }
                                break;
                            default:
                                $param[0] = $key;
                                $param[1] = $value[0];
                                $param[2] = $value[1];
                                call_user_func_array([$query, $method], $param);
                                break;
                        }
                    } else {
 ...
 ean);
    ...
    /**
     * Add many "order by" clause to the query.
     * 添加多个排序字段到查询语句中
     *
     * @author 406764368@qq.com
     * @version 2016年12月20日 11:09:29
    ...
    public function order($orderStr)
    {
        ...
        foreach ($orderStr as $value) {
            $tempOrder = explode(' ', $value);
            if ( count($tempOrder) == 2 ) {
                $this->{$this->unions ? 'unionOrders' : 'orders'}[] = [
                    'column' => trim($tempOrder[0]),
                    'direction' => strtolower(trim($tempOrder[1])) == 'asc' ? 'asc' : 'desc',
                ];
            } else {
                /* 只传字段名的情况暂不处理 */
                continue;
